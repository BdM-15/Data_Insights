# usaspending_kbr_subawards_from_db.py
"""
Extracts all subawards for KBR and affiliates from s3_processed.usaspending_subawards
and writes them to s3_processed.kbr_usaspending_subawards, preserving all columns.

- Filters by subawardee_uei (from provided UEI list)
...
# --- Logging Setup ---
def setup_logging(log_file: str = 'logs/usaspending_kbr_subawards_db.log'):
    log_dir = os.path.dirname(log_file)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    logger = logging.getLogger("usaspending_kbr_subawards_db")
...
def main():
    ...
    cursor.execute("DROP TABLE IF EXISTS s3_processed.usaspending_subawards_kbr;")
    conn.commit()
    # Create destination table with same structure
    cursor.execute(f"""
        CREATE TABLE s3_processed.usaspending_subawards_kbr AS SELECT * FROM s3_processed.usaspending_subawards WHERE false;
    """)
    conn.commit()
    # Fetch all matching subawards (no duplicates)
    logger.info("Querying source table for KBR UEIs in subawardee_uei or subawardee_parent_uei...")
    sql_uei = ','.join(['%s'] * len(UEI_LIST))
    # Prefer DISTINCT ON subaward_id if available, else use DISTINCT on all columns
    if 'subaward_id' in columns:
        distinct_select = f"DISTINCT ON (subaward_id) {col_str}"
    else:
        distinct_select = f"DISTINCT {col_str}"
    query = f"SELECT {distinct_select} FROM s3_processed.usaspending_subawards WHERE subawardee_uei IN ({sql_uei}) OR subawardee_parent_uei IN ({sql_uei})"
    cursor.execute(query, UEI_LIST + UEI_LIST)
    rows = cursor.fetchall()
    logger.info(f"Fetched {len(rows)} unique subaward records for KBR UEIs.")
    # Insert into destination table
    if rows:
        insert_query = f"INSERT INTO s3_processed.usaspending_subawards_kbr ({col_str}) VALUES ({','.join(['%s']*len(columns))})"
        for row in tqdm(rows, desc="Inserting records"):
            cursor.execute(insert_query, row)
        conn.commit()
        logger.info(f"Inserted {len(rows)} records into s3_processed.usaspending_subawards_kbr.")
    else:
