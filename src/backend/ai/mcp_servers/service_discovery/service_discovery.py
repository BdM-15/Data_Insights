...
class ServiceDiscovery:
    ...
    def discover_available_servers(self) -> Dict[str, MCPServer]:
        """
        ...
        # Add orchestrator manually to avoid self-discovery timeout
        orchestrator_capabilities = [
            ServiceCapability(
                ...
                method="POST",
                parameters={"prompt": "str", "user_id": "str", "session_id": "str", "page": "str", "tab": "str", "context": "dict"},
                examples=["Show me contract data for the last quarter", "Generate a visualization of spending trends"]
            ),
            ServiceCapability(
                name="tool_discovery",
                ...
                method="POST",
                parameters={},
                examples=["What tools are available?", "Show me all system capabilities"]
            ),
            ServiceCapability(
                name="system_status",
                ...
                method="GET",
                parameters={},
                examples=["Check system health", "Show server status"]
            ),
            ServiceCapability(
                name="legacy_orchestration",
                description="Legacy routing for backward compatibility",
                endpoint="http://localhost:8001/legacy/route",
                method="POST",
                parameters={"prompt": "str", "user_id": "str", "session_id": "str"},
                examples=["Legacy routing support"]
            )
        ]
        ...
        orchestrator_server = MCPServer(
            ...
            metadata={"name": "orchestrator", "status": "healthy", "service_type": "orchestrator"}
        )
        discovered_servers["orchestrator"] = orchestrator_server
        logger.info(f"Added orchestrator server with {len(orchestrator_capabilities)} capabilities")
        
        # Simple sequential discovery for other servers
        for port in [8002, 8003]:  # Skip 8001 (orchestrator) to avoid timeout
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Health check OK for port {port}: {health_data}")
                    ...
                    if caps_response.status_code == 200:
                        ...
                        logger.info(f"Capabilities OK for port {port}: {len(caps_data.get('capabilities', []))} capabilities")
                        ...
                        logger.warning(f"Capabilities failed for port {port}: {caps_response.status_code}")
                    ...
                    logger.warning(f"Health check failed for port {port}: {response.status_code}")
    ...
    def get_available_tools(self) -> List[Dict[str, Any]]:
        ...
        """
        tools = []
        logger.info(f"get_available_tools: Found {len(self.servers)} servers")
        for server_name, server in self.servers.items():
            logger.info(f"Server {server_name}: {len(server.capabilities)} capabilities")
        ...
        logger.info(f"get_available_tools: Returning {len(tools)} tools")
        return tools
    
    def get_server_for_capability(self, capability_name: str) -> Optional[MCPServer]:
        """
