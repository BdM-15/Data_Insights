    ...
    with engine.connect() as connection:
        ...
        if destination_exists:
            if force_rebuild:
                ...
                logger.warning("All existing data in usaprime_cleaned will be lost.")
                ...
                try:
                    ...
                    connection.commit()
                    logger.info("Successfully dropped usaprime_cleaned table.")
                except Exception as e:
                    logger.error(f"Error dropping table: {e}")
                    raise
            else:
                logger.info("usaprime_cleaned table already exists. Exiting to prevent data loss.")
                return False
    
    try:
        # Create destination table with optimized structure
        logger.info("Creating optimized usaprime_cleaned table structure...")
        with engine.connect() as connection:
            ...
            column_check_query = text("""
                ...
                WHERE 
                    ...
                    AND column_name = 'contract_transaction_unique_key'
            """)
            ...
            key_check_query = text("""
                SELECT 
                    COUNT(*) - COUNT(DISTINCT contract_transaction_unique_key) AS duplicate_count,
        ...
        with engine.connect() as connection:
            ...
            # Modified SQL transformation with PRIMARY KEY constraint (may cause issues with duplicates)
            # Will handle duplicate records at the deduplication step
            try:
                # First try using PRIMARY KEY constraint (will fail if duplicates or NULLs exist)
                transform_sql = text("""
                    ...
                    SELECT DISTINCT ON (contract_transaction_unique_key)
                        ...
                        -- Standardize NAICS code format (remove trailing .0)
                        SUBSTRING(REGEXP_REPLACE(naics_code::text, '\\.0$', ''), 1, 6),
                        naics_description,
                        product_or_service_code,
                        product_or_service_code_description,
                        dod_acquisition_program_description,
                        -- Standardize agency/organization names for consistency
                        COALESCE(NULLIF(TRIM(parent_award_agency_name), ''), 'DEPT OF DEFENSE'),
                        ...
                        -- Convert number_of_offers_received to integer if possible
                        CASE 
                ...
                # Check if there were NULL keys and handle them separately with generated keys
                if null_key_count > 0:
                    logger.warning(f"Handling {null_key_count} rows with NULL transaction keys...")
                    
                    # Create a temporary table for NULL key records
                    connection.execute(text("""
                        CREATE TEMP TABLE null_key_records AS
                        SELECT * FROM usaspending_prime_awards
                        WHERE contract_transaction_unique_key IS NULL
                    """))
                    
                    # Add a temporary sequential ID to use for generating unique keys
                    connection.execute(text("""
                        ALTER TABLE null_key_records ADD COLUMN temp_id SERIAL
                    """))
                    
                    # Insert NULL key records with generated keys
                    null_records_sql = text("""
                        INSERT INTO usaprime_cleaned
                        SELECT 
                            ...
                            parent_award_id_piid,
                            award_id_piid,
                            -- Standardize modification_number
                            CASE 
                                WHEN modification_number IS NULL THEN '0'
                                WHEN TRIM(modification_number::text) = '' THEN '0'
                                ELSE TRIM(modification_number::text)
                            ...
                            fair_opportunity_limited_sources,
                            other_than_full_and_open_competition,
                            -- Convert number_of_offers_received to integer
                            CASE 
                                WHEN number_of_offers_received ~ '^[0-9]+(\\.0+)?$' THEN 
                                    CAST(REGEXP_REPLACE(number_of_offers_received, '\\.0+$', '') AS INTEGER)
                                ELSE NULL
                ...
                # If PRIMARY KEY constraint failed, try a different approach to handle duplicates
                logger.warning(f"Error during INSERT with PRIMARY KEY constraint: {str(e)}")
                logger.warning("Dropping and recreating table without PRIMARY KEY constraint, will handle duplicates later...")
                
                # Drop the table and recreate without PRIMARY KEY constraint
                connection.execute(text("DROP TABLE IF EXISTS usaprime_cleaned"))
                connection.commit()
                ...
                # Now insert all data without PRIMARY KEY constraint
                transform_sql = text("""
                    INSERT INTO usaprime_cleaned
                    SELECT 
                        contract_transaction_unique_key,
                ...
                # Create a unique index after the fact (which will be more permissive with NULLs)
                logger.info("Creating unique index on contract_transaction_unique_key...")
                connection.execute(text("""
                    CREATE UNIQUE INDEX idx_contract_transaction_unique_key 
                    ON usaprime_cleaned(contract_transaction_unique_key)
                    WHERE contract_transaction_unique_key IS NOT NULL
                """))
                connection.commit()
            ...
            if processed_rows != source_row_count:
                ...
                null_count_query = text("SELECT COUNT(*) FROM usaprime_cleaned WHERE contract_transaction_unique_key IS NULL")
