...
def calculate_fiscal_year_quarter(dates):
    """Calculate fiscal year and quarter based on government fiscal year (Oct 1 - Sep 30)"""
    months = dates.dt.month
    years = dates.dt.year
    
    # Fiscal year starts on October 1, so months >= 10 belong to the next fiscal year
    fiscal_years = years + (months >= 10).astype(int)
    
    # Directly map months to quarters using a dictionary
    ...
    fiscal_quarters = months.map(month_to_quarter)
